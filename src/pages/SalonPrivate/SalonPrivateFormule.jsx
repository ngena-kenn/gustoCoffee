      ...
      <div className="list_forfaits-cards">
        {formules.map((forfait) => (
          <div key={forfait.id} className="list_forfaits-card">
            <h3>
              Forfait <span>{forfait.nomFormule}</span>
            </h3>
            <h3>{forfait.prix} €</h3>
            <p>{forfait.descriptionFormule}</p>
            <p>{forfait.description2}</p>
            <Link
              to="/reserver/salons-private/formules/dates"
              onClick={() => handleFormuleSelection(forfait.id)}
            >
              Choisir
            </Link>
          </div>
...
export default SalonPrivateFormule;
